...
def add_comment(request: Request, post_id):
    ...
    if new_comment.is_valid():
        ...
        dataResponse = {"msg" : "couldn't create a Comment , you have something errors"}
...
def  delete_comment(request: Request, comment_id):
    ...
    if not user.is_authenticated or user != comment.user:
        return Response({"msg" : "Not Allowed , You must be Logged in or you are not owner of the comment"}, status=status.HTTP_401_UNAUTHORIZED)
